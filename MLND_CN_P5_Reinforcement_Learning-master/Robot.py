...
class Robot(object):
    ...
    def update_Qtable(self, r, action, next_state):
        ...
        if self.learning:
            ...
            # to the given rules
            max_r = float(max(self.Qtable[next_state].values()))
            next_action = (1 - self.alpha) * self.Qtable[self.state][action] + self.alpha * (r + self.gamma * max_r)
            self.Qtable[self.state][action] = next_action
