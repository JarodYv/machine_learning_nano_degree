...
本实验是同一批受试者在两种不同文字条件下进行实验，两组样本数据数目相同，且较少（n=24）。两组样本数据来自两个服从正态分布或近似正态分布的总体（总体标准差未知），抽样分布满足t分布。这种情况适用于配对样本（Paired samples）相依t检验（Dependent t-test)。相依t检验非常适用于同一受试者在两种不同条件、或前后2种测试，或纵向研究中，分析前后两组数据的差异。
