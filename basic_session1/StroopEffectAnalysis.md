...
本实验只有24组数据(n=24)，因此我们有的只是样本数据，总体的标准差$\sigma$未知，因此需要选择**t检验**。

本实验是同一批受试者在两种不同文字条件下进行实验，两组数据一一对应，因此选择**相依t检验**来分析比较两种不同文字条件下的统计差异。
...
1. 两组不同条件下观测的样本数据
2. 两组观测数据是连续数据，且数量相同，一一对应（顺序不能变）
3. 每组观察之间是相互独立的
4. 两组样本数据来自两个服从正态分布或近似正态分布的总体，并且标准差近似。


两组样本数据数目相同，且较少（n=24）。两组样本数据来自两个服从正态分布或近似正态分布的总体（总体标准差未知），抽样分布满足t分布。这种情况适用于配对样本（Paired samples）相依t检验（Dependent t-test)。相依t检验非常适用于同一受试者在两种不同条件、或前后2种测试，或纵向研究中，分析前后两组数据的差异。
